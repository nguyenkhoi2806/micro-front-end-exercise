...
const FallbackComponent = () => {
  return <div style={{ width: "100%", height: "100%", backgroundColor: "#cccccc" }} />;
};
...
export const mountRemoteComponent = (params: Params) => {
  ...
  useEffect(() => {
    if (divRef.current) {
      if (errorLoading) {
        divRef.current.innerText = `Error loading remote module: "${String(
          module
        )} - ${component}"`;
      }
      ...
      if (ready) {
        ...
        Comp.then((mountComponent) => {
          return props
            ? mountComponent.default(divRef.current, props)
            : mountComponent.default(divRef.current);
        });
      }
    }
  ...
  return (
      <>
        {errorLoading && !ready && FallbackComponent ? (
          <FallbackComponent />
        ) : (
          <div className="mount-slot" ref={divRef} />
        )}
      </>
    )
};
